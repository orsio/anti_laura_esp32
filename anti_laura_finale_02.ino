...
//    Serial.println("Init TFT and Touch...");
  ...
  tag[0][0]  = 62;   tag[0][1]  = 241;   tag[0][2]  = 212;   tag[0][3]  = 131;   tag[0][4]  = 1;  // attiva
  tag[1][0]  = 60;   tag[1][1]  = 14;    tag[1][2]  = 72;    tag[1][3]  = 115;   tag[1][4]  = 1;
  tag[2][0]  = 94;   tag[2][1]  = 218;   tag[2][2]  = 69;    tag[2][3]  = 115;   tag[2][4]  = 1;
  tag[3][0]  = 39;   tag[3][1]  = 47;    tag[3][2]  = 44;    tag[3][3]  = 185;   tag[3][4]  = 1;
  tag[4][0]  = 57;   tag[4][1]  = 218;   tag[4][2]  = 69;    tag[4][3]  = 115;   tag[4][4]  = 1;
  tag[5][0]  = 54;   tag[5][1]  = 124;   tag[5][2]  = 148;   tag[5][3]  = 121;   tag[5][4]  = 1;
  tag[6][0]  = 189;  tag[6][1]  = 0;     tag[6][2]  = 80;    tag[6][3]  = 115;   tag[6][4]  = 2;    // tag per setup
  tag[7][0]  = 12;   tag[7][1]  = 63;    tag[7][2]  = 188;   tag[7][3]  = 169;   tag[7][4]  = 1;    // scheda bianca normale 
...
void pagina_meteo()  {    // pagina per il display delle informazioni meteo
  ...
  String hh = da.substring(11,13);
      ...
      tft.setTextColor (WHITE); 
      tft.setTextSize  (2); tft.print  ("Dato delle ore: ");  tft.println (hh);


      tft.setTextSize  (3); tft.println ("");
        
      tft.setTextSize  (2); tft.setTextColor (WHITE);  tft.print("Temp.  ");  
      tft.setTextSize  (5); tft.setTextColor (RED);  tft.println(myObject["list"][0]["main"]["temp"]);
      tft.setTextSize  (2); tft.println  ("");
      tft.setTextSize  (2); tft.setTextColor (YELLOW);  tft.println(myObject["list"][0]["weather"][0]["description"]);
      tft.setTextSize  (2); tft.println  ("");
      tft.setTextColor (WHITE);  tft.print("Pressione  ");
      tft.setTextSize  (3); tft.setTextColor (GREEN);  tft.println(myObject["list"][0]["main"]["pressure"]);
      tft.setTextSize  (2); tft.println  ("");
      tft.setTextColor (WHITE);  tft.print("Umidita %  ");
      tft.setTextSize  (3); tft.setTextColor (GREEN);  tft.println(myObject["list"][0]["main"]["humidity"]);
      tft.setTextSize  (2); tft.println  ("");
      tft.setTextColor (WHITE);  tft.print("Vento mt/s ");
      tft.setTextSize  (3); tft.setTextColor (GREEN);  tft.println(myObject["list"][0]["wind"]["speed"]);
      tft.setTextSize  (2);
...
void pagina_previsioni()  {    // pagina per il display delle previsioni meteo viene attivata dalla  pagina meteo
 ...
 //  Serial.println (myObject);
  ...
  tft.fillScreen   (NAVY);  tft.setCursor    (40, 2);
...
for (x=0; x<=16; x++) {
  ...
  if ( ore > 5 && ore < 23 && righe < 7)  {                   // evito di stampare le ore notturne 
       tft.setTextColor (WHITE);    tft.setTextSize  (2);        
           if (giorno==oggi) {tft.print ("Oggi   H:"); }
             else            {tft.print ("Domani H:"); }

       tft.setTextColor (GREEN); tft.print (hhx); tft.setTextColor (WHITE); 
       tft.print (" T=");
       tft.setTextColor (RED);       
       tft.println  (myObject["list"][x]["main"]["temp"]); 
